...
export default function SchoolTransparencyPage({params}:{params:Promise<{id:string}>}){
  ...
  if(loading)return <main style={shell}><p>Loading school finances…</p></main>;
  if(error||!data?.school)return <main style={shell}><a href="/">← Skolo Saka</a><h1>School finances unavailable</h1><p>{error||'School not found.'}</p></main>;
  return <main style={shell}>
    <a href="/" style={{fontWeight:800}}>← Skolo Saka</a>
    <header style={{margin:'24px 0 28px'}}><span style={eyebrow}>School transparency</span><h1 style={{fontSize:42,margin:'8px 0'}}>{data.school.name}</h1><p style={{fontSize:18,color:'#4b5f55'}}>{data.school.town||data.school.municipality||data.school.province} · {data.school.verified?'Verified school':'School'}</p></header>
    <section style={metrics}>
      ...
      <Metric label="Open projects" value={String(data.totals.project_count)} note="Visible school projects"/>
    </section>
