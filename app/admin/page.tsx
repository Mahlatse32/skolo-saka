...
export default function AdminPage(){
  ...
  async function load(){
    ...
    const {data:r}=await supabase.from('school_admins').select('*, schools(id,name,level,province,town)').eq('status','active');
    const adminRoles=(r??[]) as unknown as AdminRole[]; setRoles(adminRoles);
    if(!adminRoles.length){setMessage('Your account does not currently have an active school administration role.');setLoading(false);return;}
  ...
  return <main style={{minHeight:'100vh',background:'#f5f8f6',padding:'32px 18px'}}>
    <div style={{maxWidth:980,margin:'0 auto'}}>
      ...
      <header style={{display:'flex',alignItems:'center',gap:12,margin:'30px 0'}}><span className="brand-mark"><GraduationCap size={20}/></span><div><span className="eyebrow">Governance</span><h1 style={{margin:'3px 0',fontSize:34}}>School administration</h1></div></header>
