...
export async function GET(request:NextRequest){
  try{
    ...
    const {data:commitment,error:commitmentError}=await db.from('commitments').select('id,user_id,school_id,amount_cents,currency').eq('id',commitmentId).maybeSingle();
    ...
    const {error:updateError}=await db.from('commitments').update({status:'active',payment_provider:'paystack',provider_reference:data.reference,started_at:new Date(data.paid_at||Date.now()).toISOString(),cancelled_at:null,updated_at:new Date().toISOString()}).eq('id',commitment.id);
    if(updateError) throw updateError;
    const {error:ledgerError}=await db.from('ledger_transactions').upsert({user_id:commitment.user_id,school_id:commitment.school_id,commitment_id:commitment.id,type:'contribution',amount_cents:data.amount,currency:data.currency,external_reference:data.reference,metadata:{provider:'paystack',verified:true,transaction_id:data.id,channel:data.channel||null},occurred_at:new Date(data.paid_at||Date.now()).toISOString()},{onConflict:'external_reference'});
