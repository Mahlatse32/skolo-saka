...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data:commitment, error } = await supabase
      .from('commitments')
      .select('id,user_id,school_id,amount_cents,currency,status,schools(name)')
      .eq('id', commitmentId).eq('user_id', user.id).single();
    ...
    const plan = await paystackRequest<PlanResponse>('/plan', {
      method:'POST',
      body:JSON.stringify({
        name:`Skolo Saka – ${school?.name || 'School'} – R${Number(commitment.amount_cents)/100}/month`,
        amount:Number(commitment.amount_cents), interval:'monthly', currency:'ZAR',
        description:'Monthly Skolo Saka school contribution', send_invoices:true, send_sms:false,
      }),
    });
    ...
    const initialized = await paystackRequest<InitializeResponse>('/transaction/initialize', {
      method:'POST',
      body:JSON.stringify({
        email, amount:Number(commitment.amount_cents), currency:'ZAR', plan:plan.data.plan_code,
        callback_url:`${origin}/payment/complete`,
        metadata:{ commitment_id:commitment.id, user_id:user.id, school_id:commitment.school_id, source:'skolo_saka' },
      }),
    });
    await supabase.from('commitments').update({
      payment_provider:'paystack', provider_reference:initialized.data.reference, updated_at:new Date().toISOString()
    }).eq('id',commitment.id).eq('user_id',user.id);
