...
export default function PaymentsPage() {
  ...
  useEffect(() => { setConsent(false); }, [kind, term, customTerm, selected, amounts, linkLater, draftId]);
  ...
  function linkDraft(row: Instruction) {
    setDraftId(row.id); setLinkLater(false); setKind(row.kind);
    ...
    setMessage('Select your schools and review the total before confirming checkout. Your saved arrangement has not been charged.');
  ...
  async function saveArrangement() {
    ...
    try {
      const response = await authedFetch('/api/payments/drafts', { method: 'POST', body: JSON.stringify({ kind, amountCents: total, termMonths: resolvedTermMonths() }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Could not save arrangement.');
      await load();
      setMessage('Arrangement saved. Link a school whenever you are ready. No payment has been collected.');
    } catch (e) { setError(e instanceof Error ? e.message : 'Could not save arrangement.'); }
  ...
  return <main className={styles.page}>
    <div className={styles.shell}>
      ...
      <header className={styles.compactHero}><span className={styles.eyebrow}>Payments</span><h1>Support starts here.</h1><p>Set your amount and schedule now. You can link your schools later.</p></header>
      {loading && <p role="status">Loading your saved arrangements…</p>}
      {overview && overview.paymentMode !== 'live' && <div className={styles.notice} role="status">{overview?.paymentMode === 'test' ? 'Demo checkout: test payments only. No real contribution will be collected.' : 'Checkout is not available yet. You can still save a payment arrangement.'}</div>}
      ...
      <div className={styles.layout}>
        <section className={styles.card} id="payment-setup">
          <div className={styles.sectionTitle}><div><h2>Set up a payment</h2><p>Start from R10. Saving an arrangement does not charge your card.</p></div></div>
          ...
          {!draftId && <div className={styles.toggle} aria-label="When to link schools">
            <button aria-pressed={linkLater} className={linkLater ? styles.active : ''} onClick={() => setLinkLater(true)}>Link schools later</button>
            <button aria-pressed={!linkLater} className={!linkLater ? styles.active : ''} onClick={() => setLinkLater(false)}>Choose schools now</button>
          </div>}
          {draftId && <p className={styles.notice}>Linking your saved arrangement. <button type="button" onClick={() => { setDraftId(null); setLinkLater(true); }}>Back to new arrangement</button></p>}
          {linkLater ? <div className={styles.termRow}><label htmlFor="draft-amount">{kind === 'recurring' ? 'Monthly amount (R)' : 'Once-off amount (R)'}</label><input id="draft-amount" className={styles.amount} type="number" min="10" max="1000000" step="0.01" value={draftAmount} onChange={e => setDraftAmount(e.target.value)}/></div> : <div className={styles.schools}>
            ...
            {!loading && !overview?.schools.length && <div className={styles.empty}>No schools available yet. Add a school, then return here to link your saved arrangement.</div>}
          <a className={styles.back} href="/schools">Find and add a school →</a></div>}
        </section>

        <aside className={styles.summary}>
          <div className={styles.summaryBox}>
            <h3>{linkLater ? 'Your arrangement' : 'Payment summary'}</h3>
            <div className={styles.summaryLine}><span>Type</span><strong>{kind === 'recurring' ? 'Monthly' : 'Once-off'}</strong></div>
            <div className={styles.summaryLine}><span>Schools</span><strong>{linkLater ? 'Link later' : selectedSchools.length}</strong></div>
            ...
            {linkLater ? <>
              <p className={styles.summaryHint}>Save now, link schools later. No card details or charge yet. You will confirm payment securely when you are ready.</p>
              <button className={styles.primary} onClick={saveArrangement} disabled={busy || loading || !overview || total < 1000 || total > 100_000_000}>{busy ? 'Saving…' : 'Save payment arrangement'}</button>
            </> : <>
              <p className={styles.summaryHint}>{kind === 'recurring' ? 'Your card is charged at checkout, then monthly. Contributions are allocated to your selected schools.' : 'Your card is charged once at checkout. Your contribution is allocated to your selected schools.'}</p>
              ...
              <button className={styles.primary} onClick={startPayment} disabled={busy || loading || !consent || !selectedSchools.length || !overview || overview.paymentMode === 'unavailable'}>{busy ? 'Opening secure checkout…' : 'Continue to secure checkout'}</button>
            </>}
          </div>
        </aside>
      </div>
      ...
      <section className={styles.activeSection}>
        ...
        <div className={styles.paymentGrid}>
          {currentInstructions.map(row => <article className={styles.paymentCard} key={row.id}><div className={styles.paymentHead}><div><h3>{row.kind === 'recurring' ? 'Monthly arrangement' : 'Once-off arrangement'}</h3><div className={styles.legacyTag}>{row.provider === 'draft' ? 'No card linked · No charge' : row.provider === 'paystack' ? 'Paystack' : row.provider}</div></div><span className={`${styles.status} ${row.status === 'pending' ? styles.pending : row.status === 'non_renewing' ? styles.nonrenewing : ''}`}>{row.provider === 'draft' ? 'Awaiting school' : row.status.replace('_',' ')}</span></div><div className={styles.paymentMeta}><span><b>{money(row.amount_cents)}</b>{row.kind === 'recurring' ? '/month' : ''}</span>{row.kind === 'recurring' && <span>{row.term_months ? `${row.term_months} month term` : 'Until cancelled'}</span>}</div>{row.next_payment_at && <p className={styles.fineprint}>Next payment: {new Date(row.next_payment_at).toLocaleDateString('en-ZA')}</p>}<div className={styles.allocations}>{row.allocations.map(a => <div className={styles.allocation} key={a.id}><span>{a.school?.name || 'School'}</span><span>{money(a.amount_cents)}</span></div>)}</div>{row.provider === 'draft' && <button className={styles.linkButton} disabled={busy || loading} onClick={() => linkDraft(row)}>Link schools</button>}{(row.kind === 'recurring' || row.provider === 'draft') && <button className={styles.danger} disabled={cancelling === row.id || row.status === 'non_renewing'} onClick={() => cancelInstruction(row.id)}>{row.status === 'non_renewing' ? 'Cancellation requested' : cancelling === row.id ? 'Cancelling…' : row.provider === 'draft' ? 'Delete arrangement' : 'Cancel monthly payment'}</button>}</article>)}
      ...
      {!!contributions?.length && <section className={styles.historySection}><h2>Contributions received</h2><p className={styles.fineprint}>Your latest 100 school allocations, before processing fees. A combined payment appears once for each supported school.</p><div className={styles.historyCard}>{contributions!.map(row=><div className={styles.historyRow} key={row.id}><div><b>{(Array.isArray(row.schools)?row.schools[0]:row.schools)?.name || 'School contribution'}</b><small>{new Date(row.occurred_at).toLocaleDateString('en-ZA')}</small></div><span className={styles.historyStatus}>Received</span><span className={styles.historyAmount}>{money(row.amount_cents)}</span></div>)}</div></section>}
